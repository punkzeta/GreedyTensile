...
alpha = -4.4732
beta = 13.0715
def get_l_con(G, G_pos, avg_degree, src, dst):

  delta =  get_two_nodes_dist(G, G_pos, src, dst)

  if delta < 1:
    l_con = 1.
  else:
    l_con = max(1+ (alpha * math.log(delta)+ beta)/((avg_degree+1)**2), l_con_min)

  return l_con

def estimate_q_value(model, G, G_pos, avg_degree, v, u, src, dst):

  lr = 0.1
  gamma = 1.0
  reward = get_two_nodes_dist(G, G_pos, v, u)
  # reward = get_stretch_factor(G, G_pos, src, dst, u)

  estimated_path_stretch = get_l_con(G, G_pos, avg_degree, src, dst)
  search_factor = (1+margin)*estimated_path_stretch
  node_stretch = get_stretch_factor(G, G_pos, src, dst, u)
  ...
  for w in G[u]:
    x_data = torch.Tensor(get_x_data(G, G_pos, u, w, src, dst)).to(dev)
  ...
  if u == dst:
    ...
    ori_x_data = torch.Tensor(get_x_data(G, G_pos, v, u, src, dst)).to(dev)
    # print("({}, {}) = {}".format(v, u, ori_x_data))
    ori_q = model(ori_x_data).detach().numpy()[0]
    # print("original q = ", ori_q)

    if node_stretch < search_factor and dist <= get_two_nodes_dist(G, G_pos, v, dst):
...
def generate_sanity_dataset(model, G, G_pos, avg_degree, dst):
  dataset_x = list()
  dataset_y = list()
  ...
  # if 1:
    ...
    for edge in G.edges:
      # if edge in all_pairs_sp[src][dst]:
        ...
        y_data = [estimate_q_value(model, G, G_pos, avg_degree, v, u, src, dst)]
...
def generate_examples_selected(model, G, G_pos, avg_degree, src_list, dst):
  dataset_x = list()
  dataset_y = list()
...
while(done == False):
  ...
  try:
    for round in range(num_of_rounds):
      ...
      X_train, Y_train = generate_examples_selected(model, train_G, train_G_pos, Degree, src_list, Destination)
